...
function getManagedDistributors(): Distributor[] {
  try {
    const stored = localStorage.getItem("aurex-data-distributors")
    return stored ? JSON.parse(stored) as Distributor[] : defaultDistributors
  } catch {
...
export default function DistributorsPage() {
  ...
  const [distributors, setDistributors] = useState<Distributor[]>(defaultDistributors)
  ...
  useEffect(() => {
    setDistributors(getManagedDistributors())
  }, [])

  const wilayas = useMemo(
    () => [...new Set(distributors.map((d) => ml(d.wilaya)))].sort(),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [distributors, i18n.language],
  )
  const communes = useMemo(
    () =>
      [...new Set(distributors.filter((d) => !wilaya || ml(d.wilaya) === wilaya).map((d) => ml(d.commune)))].sort(),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [distributors, wilaya, i18n.language],
  )
  const filtered = useMemo(
    () =>
      distributors.filter(
        (d) => (!wilaya || ml(d.wilaya) === wilaya) && (!commune || ml(d.commune) === commune),
      ),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [distributors, wilaya, commune, i18n.language],
  )
  ...
  // Les filtres stockent des libellés localisés : reset au changement de langue
  useEffect(() => {
    setWilaya("")
    setCommune("")
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  return (
    <div className="min-h-screen bg-slate-50 pt-24">
      {/* ══ Header ══ */}
      <section className="relative overflow-hidden bg-[var(--color-primary)] px-6 pb-24 pt-14 text-white">
        <div
          aria-hidden="true"
          className="pointer-events-none absolute -right-24 -top-24 h-80 w-80 rounded-full bg-[var(--color-accent)]/25 blur-3xl"
        />
        <div
          aria-hidden="true"
      ...
      <main className="mx-auto max-w-7xl px-4 pb-16 sm:px-6">
          ...
          <div className="grid gap-4 lg:grid-cols-[1fr_1fr_auto_auto]">
            <label className="text-[11px] font-bold uppercase tracking-wider text-slate-400 font-sans">
              ...
              <select
                ...
                className="mt-2 w-full rounded-xl border border-slate-200 bg-slate-50/60 px-3.5 py-3 text-sm font-medium normal-case tracking-normal text-slate-800 outline-none transition focus:border-[var(--color-accent)] focus:bg-white focus:ring-4 focus:ring-blue-100 font-sans"
              >
            ...
            <div className="flex items-end">
              <p className="whitespace-nowrap rounded-xl bg-[var(--color-primary)] px-5 py-3 text-sm font-bold text-white font-sans">
        ...
        <div className="grid gap-5 lg:grid-cols-[1.45fr_0.55fr]">
          ...
          >
            ...
            <div className="pointer-events-none absolute left-4 top-4 z-[400] flex items-center gap-2 rounded-xl bg-white/95 px-3.5 py-2.5 text-xs font-semibold text-[var(--color-primary)] shadow-md backdrop-blur-sm font-sans">
            ...
            <button
              ...
              className="absolute right-4 top-4 z-[500] inline-flex items-center gap-2 rounded-xl bg-white px-3.5 py-2.5 text-xs font-bold text-[var(--color-primary)] shadow-md transition hover:bg-[#EFF3FB] font-sans"
            >
            ...
            {isFullscreen && selected && (
              <div className="absolute bottom-4 left-4 right-4 z-[500] max-w-md rounded-2xl border border-white/60 bg-white/95 p-4 shadow-2xl backdrop-blur-md sm:left-auto sm:right-6 sm:w-[360px]">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="text-base font-bold text-[var(--color-primary)] font-display">
                        {ml(selected.name)}
                      </p>
                      <p className="mt-1 text-xs text-slate-500 font-sans">
                        {ml(selected.commune)} · {ml(selected.wilaya)}
                      </p>
                    </div>
                    <button
                      type="button"
                  ...
                  <div className="mt-3 space-y-1.5 text-xs text-slate-600 font-sans">
                    <p>{ml(selected.address)}</p>
                    <a href={`tel:${selected.phone}`} className="block text-[var(--color-accent)]">
                      {selected.phone}
                    </a>
                    {selected.email && (
                      <a href={`mailto:${selected.email}`} className="block break-all text-[var(--color-accent)]">
                        {selected.email}
                      </a>
                    )}
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="mt-3 inline-flex w-full items-center justify-center rounded-xl bg-[var(--color-primary)] px-4 py-2.5 text-xs font-bold text-white transition hover:bg-[#12348f] font-sans"
                  >
          ...
          <aside className="flex max-h-[560px] flex-col overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm">
            {selected ? (
              <>
                {/* En-tête détail */}
                <div className="relative bg-gradient-to-br from-[var(--color-primary)] to-[#12348f] px-5 pb-8 pt-5 text-white">
                  <button
                    type="button"
                    onClick={() => setSelected(null)}
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <h2 className="text-xl font-bold leading-snug font-display">{ml(selected.name)}</h2>
                    <span className="shrink-0 rounded-lg bg-white/15 px-2.5 py-1 text-xs font-bold">
                      {String(wilayaCode[mlFr(selected.wilaya)] ?? "").padStart(2, "0")}
                    </span>
                  </div>
                  <p className="mt-1.5 text-xs text-blue-200">{ml(selected.commune)} · {ml(selected.wilaya)}</p>
                </div>

                {/* Corps détail */}
                <div className="flex-1 space-y-3 overflow-y-auto px-5 py-5 text-sm text-slate-600 font-sans">
                  <div className="flex items-start gap-3 rounded-xl bg-slate-50 p-3">
                    <svg className="mt-0.5 h-4.5 w-4.5 h-5 w-5 shrink-0 text-[var(--color-accent)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z" />
                      <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z" />
                    </svg>
                    <p className="leading-relaxed">{ml(selected.address)}</p>
                  </div>
                  <a href={`tel:${selected.phone}`} className="flex items-center gap-3 rounded-xl bg-slate-50 p-3 transition hover:bg-blue-50">
                    <svg className="h-5 w-5 shrink-0 text-[var(--color-accent)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="mt-2 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[var(--color-primary)] px-4 py-3.5 text-sm font-bold text-white shadow-md shadow-blue-900/20 transition hover:-translate-y-0.5 hover:bg-[#12348f] hover:shadow-lg font-sans"
                  >
                    <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              <>
                <div className="border-b border-slate-100 px-5 pb-4 pt-5">
                  <h2 className="text-lg font-bold text-[var(--color-primary)] font-display">
                      ...
                      return (
                        <button
                          ...
                          className={`group w-full rounded-xl border p-3.5 text-start transition-all ${
                            isActive
                              ? "border-[var(--color-accent)] bg-blue-50/60 ring-2 ring-blue-100"
                              : "border-slate-100 hover:border-blue-200 hover:bg-slate-50 hover:shadow-sm"
                          }`}
                        >
                          <div className="flex items-center justify-between gap-2">
                            <span className="text-sm font-semibold text-slate-800 transition-colors group-hover:text-[var(--color-primary)] font-sans">
                              {ml(distributor.name)}
                            </span>
                            <svg className="h-4 w-4 shrink-0 text-slate-300 transition-all group-hover:translate-x-0.5 group-hover:text-[var(--color-accent)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                          ...
                          <span className="mt-1 flex items-center gap-1.5 text-xs text-slate-500 font-sans">
                            ...
                            {ml(distributor.commune)}, {ml(distributor.wilaya)}
                          </span>
                        </button>
                      )
                    })
